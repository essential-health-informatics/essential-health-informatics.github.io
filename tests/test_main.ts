...
const tempFilePath = `${currentDir}/temp_test_combined.ts`;
...
async function runTest(filename: string): Promise<void> {
  ...
  if (code === 0) {
    console.log("TypeScript unit test passed.");
    console.log(output);
  } else {
    console.error("TypeScript unit test failed:");
    console.log(output);
    console.error(error);
    Deno.exit(code);
  }
...
for await (const dirEntry of Deno.readDir(inputDir)) {
  if (dirEntry.isFile && dirEntry.name.endsWith("_test.ts")) {
    console.log(`Found TypeScript file: ${dirEntry.name}`);

    await runTest(`${inputDir}/${dirEntry.name}`);
